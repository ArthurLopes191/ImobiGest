...
export default function ImobiliariaModal({ showModal, onClose, onSuccess }: ImobiliariaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Erro ao criar imobiliária');
      }

      setSuccess('Imobiliária criada com sucesso!');
      resetForm();
    ...
    <div className="fixed inset-0 bg-[rgba(200,200,200,0.5)] flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full mx-4">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold text-gray-800">Nova Imobiliária</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button
              type="submit"
              disabled={isLoading || !formData.nome.trim() || formData.meta <= 0}
              className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isLoading ? 'Criando...' : 'Criar Imobiliária'}
            </button>
          </div>
        </form>
      </div>
